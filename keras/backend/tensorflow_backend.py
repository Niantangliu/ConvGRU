...
def clear_session():
    ...
    reset_uids()
    _SESSION = None
    phase = tf.compat.v1.placeholder(dtype='bool', name='keras_learning_phase')
    _GRAPH_LEARNING_PHASES = {}
...
def zeros(shape, dtype=None, name=None):
    ...
    return variable(tf.constant_initializer(0.)(shape),
                    dtype, name)
...
def ones(shape, dtype=None, name=None):
    ...
    if dtype is None:
        dtype = floatx()
    tf_dtype = tf.as_dtype(dtype)
    return variable(tf.constant_initializer(1.)(shape),
                    dtype, name)
...
def random_uniform_variable(shape, low, high, dtype=None,
    ...
    value = tf.random.uniform_initializer(
...
def dot(x, y):
    ...
    if ndim(x) is not None and (ndim(x) > 2 or ndim(y) > 2):
        ...
        yt = tf.reshape(tf.compat.v1.transpose(y, perm=y_permute_dim), [y_shape[-2], -1])
...
def batch_dot(x, y, axes=None):
    ...
    if ndim(x) == 2 and ndim(y) == 2:
        if axes[0] == axes[1]:
            ...
            out = tf.reduce_sum(tf.multiply(tf.compat.v1.transpose(x, [1, 0]), y), axes[1])
    else:
        if axes is not None:
...
def max(x, axis=None, keepdims=False):
    ...
    """
    return tf.compat.v1.reduce_max(x, axis=axis, keepdims=keepdims)
...
def min(x, axis=None, keepdims=False):
    ...
    """
    return tf.compat.v1.reduce_min(x, axis=axis, keepdims=keepdims)
...
def sum(x, axis=None, keepdims=False):
    ...
    """
    return tf.compat.v1.reduce_sum(x, axis=axis, keepdims=keepdims)
...
def prod(x, axis=None, keepdims=False):
    ...
    """
    return tf.compat.v1.reduce_prod(x, axis=axis, keepdims=keepdims)
...
def cumsum(x, axis=0):
    ...
    """
    return tf.compat.v1.cumsum(x, axis=axis)
...
def cumprod(x, axis=0):
    ...
    """
    return tf.compat.v1.cumprod(x, axis=axis)
...
def var(x, axis=None, keepdims=False):
    ...
    return tf.compat.v1.reduce_mean(devs_squared,
                          axis=axis,
                          keepdims=keepdims)
...
class Function(object):
    ...
    It's possible to pass arguments to `tf.compat.v1.Session.run()` via `session_kwargs`.
    ...
    # Arguments
        ...
        session_kwargs: arguments to `tf.compat.v1.Session.run()`: `fetches`, `feed_dict`,
...
def function(inputs, outputs, updates=None, **kwargs):
    ...
    # Arguments
        ...
        **kwargs: Passed to `tf.compat.v1.Session.run`.
    ...
    if kwargs:
        for key in kwargs:
            if not (has_arg(tf.compat.v1.Session.run, key, True) or has_arg(Function.__init__, key, True)):
...
def rnn(step_function, inputs, initial_states,
    ...
    inputs = tf.compat.v1.transpose(inputs, (axes))
    ...
    if mask is not None:
        ...
        mask = tf.compat.v1.transpose(mask, axes)
    ...
    outputs = tf.compat.v1.transpose(outputs, axes)
...
def in_train_phase(x, alt, training=None):
    ...
    elif training == 0 or training == False:
        if callable(alt):
            return alt()
        else:
            return alt
...
def random_binomial(shape, p=0.0, dtype=None, seed=None):
    ...
    if seed is None:
        seed = np.random.randint(10e6)
    return tf.where(tf.random.uniform(shape, dtype=dtype, seed=seed) <= p,
...
def ctc_label_dense_to_sparse(labels, label_lengths):
    ...
    batch_array = tf.compat.v1.transpose(tf.reshape(tf.tile(tf.range(label_shape[0]),
                                                  max_num_labels_tns), reverse(label_shape, 0)))
    batch_ind = tf.boolean_mask(batch_array, dense_mask)
    indices = tf.compat.v1.transpose(tf.reshape(concatenate([batch_ind, label_ind], axis=0), [2, -1]))
...
def ctc_batch_cost(y_true, y_pred, input_length, label_length):
    ...
    y_pred = tf.math.log(tf.compat.v1.transpose(y_pred, perm=[1, 0, 2]) + 1e-8)
